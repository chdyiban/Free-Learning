...
1.28|将之前的案例复习了一遍，并且开始做一个新的案例
1.29|1、跟着做了一个新的逻辑回归案例（信用卡欺诈），当样本不均衡时我们使用的几种方法，下采样和过采样（使用SMOTE算法）。</br>2、看了决策树算法的使用，没有学习基本算法的实现方式，只是学习sklearn中决策树的一些参数而已
1.30|1、了解一些集成算法的原理</br>2、使用随机森林做了一个泰坦尼克沉船的案例
1.31|1、学习贝叶斯算法以及朴素贝叶斯的假设</br>2、通过朴素贝叶斯做了一个文本纠错的东西</br>3、使用结巴分词对五千篇新闻进行分词，使用gensim库并且使用LDA模型进行分类，感觉这个效果还比较好，另外尝试了对文本进行了向量化以及使用tf-idf进行向量化，然后使用多项式朴素贝叶斯建立模型，划分的正确率大概为80%左右，正确率还好，但是感觉划分的特征很多都是相近的词以及数词（一部分在停用词中去掉了，但是还有一部分无法剔除），准备详细看看LDA的实现原理
2.1|看了一天LDA的数学原理，说实话没怎么看懂，只能说大概能够理解它的思想，在众多doc-topic骰子中选择一个z，然后在K个topic-word骰子中选择编号为z这个骰子，由这个骰子再来生成一个词。当应用贝叶斯的思想来说，无论是doc-topic还是topic-word骰子，他们都并非只有一个，也就是概率可变的，于是我们使用beta分布（在多项式中就是狄利克雷分布）等等这些。理解它的思想不是很难，但是理解数学还是有些难度，暂时没有准备详细的理解每一步数学推导
2.2|接着看了gensim库中LDA模型的实现，里面的一些方法。以及接触了支持向量机一些推导过程（有些公式的详细证明就省略了），这个还没有看完，明天接着看。
2.3|今天学习了支持向量机的数学推导过程（主要是物理过程），支持向量机主要是使决策边界最大化，并且引入了松弛变量（防止模型过拟合，对离群点和噪音点的容忍度更大），通过高斯核函数进行等同于升维的过程。
2.4|学习K-MEANS和DBSCAN两种聚类算法，K-MEANS分类比较快，但是准确度不是很好，所以一般需要进行多次聚类，而DBSCAN对于大多数情况下分类效果很好，但是速度太慢了，尤其是在高纬度的时候。
2.5~2.7|过年休息，哈哈哈
