...
loadFile <- function(file) {
	...
	listing$content <- factor(listing$content)
	listing$author <- factor(listing$author)
	listing_corpus <- VCorpus(VectorSource(listing$content))
...
# We're not going to calculating KNN confusionMTX here because:

# 1. Not necessary;

# 2. It take long time;

# 3. Labels for KNN can't be easily gotten.
