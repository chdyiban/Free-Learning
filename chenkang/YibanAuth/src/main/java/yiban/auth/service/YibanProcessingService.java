...
public class YibanProcessingService {
	...
	public static void run(HttpServletResponse response, UserInfo user, String targetUrl) throws Exception {
		CHDYibanUIS uis = CHDYibanUIS.getInstance();
		uis.setup(response, PRIV_FILE);
		uis.run(user.toMap(), targetUrl, false);
	}
	
	public static void run(HttpServletResponse response, UserInfo user) throws Exception {
		CHDYibanUIS uis = CHDYibanUIS.getInstance();
		uis.setup(response, PRIV_FILE);
